...
**Last updated by:** PM module build chat — built Step 3 (CPM forward-pass scheduling engine, dependency UI, cycle protection)
...
- **Current version:** v6.8 (DB schema version 6 — Step 3 needed no new stores, `pmDependencies` already existed from Step 1)
- **File list:** `index.html`, `manifest.json`, `sw.js`, `css/styles.css`, `js/db.js`, `js/bci.js`, `js/geo.js`, `js/backup.js`, `js/app.js`, `js/annotate.js`, `js/signature.js`, `js/pdf.js`, `js/pdfeditor.js`, `js/pmdate.js` (new — shared pure date-math module, see 4.1), `js/pmschedule.js` (new — pure CPM engine, no DOM, see 4.1), `js/pm.js`, `js/launcher.js`, `js/shell.js`
...
- **Status:** In Progress — Step 1 **Built — Field Tested**, Steps 2 and 3 **Built — Untested** (neither has been checked on a real device yet — see the explicit callout below)
- **Decision reversed back:** this module lives **inside Inspector**, vanilla JS, no build step — not the separate React/TS/Vite PWA the prior session had pivoted to. See the amended Section 5 entry.
- **Description:** Full build brief exists (Microsoft-Project-inspired — WBS, CPM scheduling engine, interactive Gantt, resources, calendars, milestones, baselines, cost tracking, dashboards). Agreed build sequence:
  ...
  2. **Read-only Gantt render** — ✅ built, ⚠️ not yet device-tested
  3. **CPM scheduling engine** — ✅ built this session, ⚠️ not yet device-tested
  4. Drag interaction + undo/redo — not started
- **⚠️ Sequencing note, raised explicitly by the user before this session started and still true:** Step 2 has still not been checked on a real iPad. Step 3 is built directly on top of Step 2's Gantt rendering, so if Step 2 has an undiscovered rendering bug, it's now sitting underneath new scheduling logic too. **Recommend testing Steps 2 and 3 together on a real device before treating either as done**, rather than assuming Step 2 was fine because Step 1 was.
- **Current capabilities (Steps 1–2): unchanged from the last entry — see below for what's new.**
- **Current capabilities (Step 3, built this session):**
  - **New file `js/pmdate.js`** — the date parsing/formatting/diffing that Steps 1–2 already had inline in `pm.js` was extracted into its own pure, shared module (`pmParseISODate`/`pmFormatISODate`/`pmDaysBetween`), specifically so the CPM engine could not end up with a second, subtly different date implementation. `pm.js` now uses this module rather than defining its own.
  - **New file `js/pmschedule.js`** — the CPM engine itself. Pure, framework-free, no DOM code, dual-exported (`window.PMSchedule` in browser, `module.exports` under Node) specifically so it's independently unit-testable. **Scope for this pass: Finish-to-Start (FS) dependencies with lag only.** SS/FF/SF are stored (unchanged since Step 1) but explicitly not scheduled by this pass — confirmed by a dedicated test. Late Start/Finish and total float (the backward pass) are **not** computed yet, since they need a required project finish date that nothing captures yet.
  - **Auto-scheduling behavior, per the user's explicit decision this session:** a task with an FS predecessor is pushed later automatically if its current start would violate the dependency, but is *never* pulled earlier automatically — a task scheduled later than the minimum keeps that buffer (float) until the user changes it themselves. Manually editing a task's start date is validated against its predecessors' constraints (`PMSchedule.validateManualStart`) and rejected with a clear "blocked by X, earliest is Y" message if it would violate one — it is not silently clamped.
  - **Dependency creation cycle protection**, per the user's explicit push on this: implemented **at the data layer**, inside `DB.createPMDependency` in `db.js` — mirrors the existing `movePMTask` ancestor-walk pattern, applied to the dependency graph instead of the WBS parent chain. Throws before a cycle can ever be persisted, regardless of what UI path attempts it. `PMSchedule.wouldCreateCycle` is a second, pure copy of the same check used by the engine's own tests and by the "add predecessor" picker to pre-filter obviously-invalid choices for a better UX — deliberate, low-risk duplication (a small ID-reachability check, not the kind of subtle date-math bug the user was right to worry about elsewhere), not an oversight.
  - **New Dependencies section in the task edit sheet** (leaf tasks only — summary tasks aren't independently dated and can't be dependency endpoints in this pass): lists current predecessors with any lag, a remove button per link, and an "+ Add predecessor" picker listing other leaf tasks in the project, already filtered to exclude self, already-linked tasks, and cycle-creating choices.
  - Every dependency create/remove and every task save now runs `pmRecomputeSchedule()` (the forward pass), persists any tasks it moved, and toasts "Also moved: X, Y" so cascading changes are never silent — matches the original brief's "clearly indicate the resulting change to the project completion date."
  - Fixed a real gap found while touching `sw.js` this session: **`pm.js` was never added to the offline-install precache list (`APP_SHELL`) back in Step 1.** Corrected now, along with adding the two new files — all three (`pmdate.js`, `pmschedule.js`, `pm.js`) are now precached from first install rather than only being cached lazily on first fetch.
- **Automated tests — `js/pmschedule.test.js`, run via plain `node js/pmschedule.test.js`, no browser needed:** 19 tests, all passing. Covers: date parsing/round-tripping, two real DST-transition dates (US and UK) confirming the UTC-based day math isn't affected, cycle detection (self-reference, direct cycle, indirect chain cycle, and a valid diamond-shaped graph that must NOT be flagged), a task with no predecessors never moving, basic FS scheduling, float being preserved (a task already scheduled later than required is left alone), positive lag, a 3-level cascading chain (A→B→C, confirms C reacts to B's *new* finish date, not its original one), milestone start=finish behavior, confirmation that SS/FF/SF are inert in this pass, and both branches of manual-start validation.
- **Verification done this session:**
  - `node --check` on every new/modified JS file
  - Re-ran the duplicate-top-level-declaration grep across all 14 app JS files (up from 12) — none found
  - Re-ran the full shared-scope Node `vm` load simulation with all 14 files — all clean
  - The dedicated `pmschedule.test.js` suite above (19/19 passing)
  - **Still not done: real iPad Safari testing of either Step 2 or Step 3.** Both remain `Built — Untested`. This is the single most important gap in the whole module right now — everything downstream (Step 4: drag interaction) will be harder to debug correctly if it's layered on top of two unverified prior steps.
- **Explicitly not built yet (by design):** SS/FF/SF scheduling math, backward pass (Late Start/Finish, total/free float, critical path), drag-to-reschedule, dependency arrows drawn on the Gantt chart itself (dependencies exist and are enforced, but aren't drawn as lines yet), undo/redo, resources/calendars/baselines/costs.
- **Open questions carried forward:**
  - Whether the eventual interactive Gantt (Step 4) needs a zoom control, or the fixed 20px/day density is kept
  ...
  - Critical path / float display is implied by the original brief but has no committed step yet — likely belongs alongside or after the backward pass, not decided
- **Next steps:** Get Steps 2 and 3 in front of a real iPad before starting Step 4. When Step 4 (drag interaction + undo/redo) does start, dragging a bar will need to call the same `pmRecomputeSchedule()`/`validateManualStart` functions already built here rather than inventing a parallel path — the engine doesn't care whether a date change came from a sheet or a drag gesture.
