...
- **Known bug, logged this session, NOT yet fixed (explicitly deferred — do not build until asked):** the task edit sheet's Start/Finish/Duration fields can drift out of sync with each other. The drag/resize interaction (Step 4) already keeps them consistent — moving a bar preserves duration, resizing a handle recalculates duration from the new start/finish — but the **sheet itself has no equivalent wiring**. Right now a person can change Duration without Finish updating, or change Finish without Duration updating, and the three fields silently disagree until something else (a drag, a cascade) forces a recalculation. Fix, when this gets picked up: changing Duration should recompute Finish from Start using `PMCalendar.addWorkingDays` (the same function the engine already uses); changing Finish directly should recompute Duration using `PMCalendar.countWorkingDays` (also already built, currently only used internally by the engine's own resize-adjacent logic, not wired to any UI field). Both calculations already exist and are already tested in `pmcalendar.test.js` — this is a wiring gap in `pm.js`'s sheet, not a missing calculation.
