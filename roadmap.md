...
**Last updated by:** PM module build chat — Steps 2–3 confirmed field-tested on a real iPad; built Step 4 (drag/resize on the Gantt chart, atomic undo/redo)
...
- **Current version:** v6.9 (DB schema version 6 — Step 4 needed no new stores)
...
- **Status:** In Progress — Steps 1–3 **Built — Field Tested** (confirmed on a real iPad), Step 4 **Built — Untested** (device check still pending)
- **Decision reversed back:** this module lives **inside Inspector**, vanilla JS, no build step. See the amended Section 5 entry.
- **Description:** Full build brief exists (Microsoft-Project-inspired). Agreed build sequence:
  1. **Data model + plain editable task table** — ✅ built, ✅ field-tested
  2. **Read-only Gantt render** — ✅ built, ✅ field-tested this session
  3. **CPM scheduling engine (FS + lag only)** — ✅ built, ✅ field-tested this session
  4. **Drag interaction + undo/redo** — ✅ built this session, ⚠️ not yet device-tested
- **Current capabilities (Steps 1–3): unchanged from prior entries, now confirmed working on a real iPad — the Step 2/3 device-testing gap flagged at the top of the previous entry is now closed.**
- **Current capabilities (Step 4, built this session):**
  - **Drag-to-reschedule and resize directly on the Gantt bars**, wired through the exact same `pmCommitTaskDates`/`PMSchedule.validateManualStart`/`PMSchedule.computeForwardPass` functions the task sheet already used — per the note left at the end of the Step 3 entry, there is no separate code path for "a date change from a drag" vs "a date change from a sheet edit."
  - **Move** (drag the bar body): shifts start, preserves duration, finish moves with it. **Resize left/right** (drag a handle): changes start or finish independently, recalculating duration, only shown once a bar is wide enough (≥34px, roughly a 2-day task at the current 20px/day density) to still leave a usable middle "move" zone — below that, dragging still moves the task, but precise duration changes go through the task sheet instead, which was always the reliable fallback for short tasks.
  - **Touch target sizing was decided deliberately before writing any drag logic**, per the user's explicit push on this point: the hit area is the full 44px row height (not the 24px visual bar), matching the app's own established precedent (`setPointerCapture` fixes for the annotator's color/width swatches, tolerance bands for thin annotator lines). Milestones get an even wider hit area (30px) than their 20px visual diamond, since a diamond alone is a genuinely small target.
  - **Live feedback during drag**: the dragged bar's position/width updates in real time via direct style writes (not a full re-render per pointer move — too slow to feel responsive), plus a small floating label showing the tentative date next to the finger/pointer — the closest existing precedent in the app is the annotator's magnifier, giving precision feedback during touch-driven placement.
  - **`setPointerCapture` used throughout**, consistent with the app's own established convention for reliable touch drag.
  - **Validation on release, not during drag**: if the finished drag would violate a predecessor constraint, nothing is persisted, a toast explains why (predecessor name + earliest valid date), and the bar visually snaps back via a full re-render from the untouched underlying data. The engine's "push later, never pull earlier" rule (decided in Step 3) applies identically here.
  - **Undo/redo — the specific concern raised before this step started.** Every drag or resize that triggers a forward-pass cascade is recorded as **one atomic undo entry** covering every task the action touched — the directly-dragged task and everything the cascade moved as a result — not one entry per task. Undo reverts the whole thing in a single action; redo re-applies it. The task-sheet save path was refactored to go through the same mechanism, so a manual start-date edit that cascades is equally atomic.
  - **Undo/redo scope is deliberately bounded, not a general system**: it covers date changes only — drag, resize, and manual start/finish edits via the sheet (including their cascades). It explicitly does **not** cover dependency creation/removal or structural WBS changes (indent/outdent/add/delete task) — those remain non-undoable, same as before this step. This was a scope decision, not an oversight — see "Explicitly not built yet" below.
  - Undo/Redo buttons added to the workspace toolbar, disabled when their respective stack is empty. The undo/redo stacks are in-memory per session (not persisted across app restarts) — reasonable for now, worth reconsidering if it turns out to matter in practice.
- **Verification done this session:**
  - `node --check` on the modified file
  - Duplicate-top-level-declaration grep across all 14 app JS files — none found
  - Full shared-scope Node `vm` simulation with all 14 files — clean
  - Re-ran the full `pmschedule.test.js` suite (19/19 still passing — Step 4 didn't touch the engine itself, only how `pm.js` calls it)
  - Standalone arithmetic check of the three drag outcomes (move, resize-right, resize-left) plus the resize-left overshoot-past-finish clamp, against hand-computed expected values — all correct
  - **Not yet done: real iPad Safari testing of Step 4 specifically.** Steps 1–3 are now confirmed; Step 4 is new and should be treated as `Built — Untested`, particularly the touch handling itself (pointer capture behavior and the handle hit-target sizing are exactly the kind of thing that looks right in code and still needs a real finger on real glass to confirm) and the horizontal-scroll-vs-drag interaction on the chart (dragging a bar vs. panning the chart background use different hit areas by design, but that boundary is worth confirming doesn't feel confusing in practice).
- **Explicitly not built yet:** SS/FF/SF scheduling math, backward pass (Late Start/Finish, float, critical path), dependency arrows drawn on the chart, zoom/pan controls, undo/redo for dependency changes or structural WBS edits, resources/calendars/baselines/costs.
- **Open questions carried forward:**
  ...
  - Critical path / float display — implied by the brief, no committed step yet
  - Whether undo/redo should eventually extend to dependency and structural changes, or stay scoped to dates
- **Next steps:** Get Step 4 in front of a real iPad. After that, Phase 1 of the original build sequence is complete — worth a deliberate conversation about what Phase 2 (resources, calendars, baselines, costs) looks like rather than sliding into it, same as the stack/location decision at the start of this module got its own explicit conversation rather than being assumed.
